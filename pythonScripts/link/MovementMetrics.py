...
def metricRadiusOfGyration(trace):
	...
	if len(trace) > 0:
		...
		for i in range(0, len(trace)):
			point = trace[i]
			sum = sum + (times[point[0]] * math.pow(distance(point[0][0], point[0][1], centroid[0], centroid[1]), 2))
...
def metricMaximumDistanceFromHome(fullTrace, trace):
	...
	for i in range(0, N):
		dist = distance(trace[i][0][0], trace[i][0][1], home[0], home[1])
		if dist > maxDist:
			maxDist = dist
